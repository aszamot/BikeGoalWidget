...
`UiText` (`core`) wraps strings that originate from — or could originate from — a string resource:
...
Define `DataError.toUiText()` extension functions in `core` (or feature `presentation`) that map error enums to `UiText.StringResource`.
...
Receives the ViewModel (via `hiltViewModel()`) and any callbacks needed for navigation.
